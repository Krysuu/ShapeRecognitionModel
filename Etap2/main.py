...
# Architektura modelu: xception, resnet50, vgg16
architecture = sys.argv[3].lower()  #

# Typ importowania gotowego modelu
# 0 - import wag z możliwością edycji
# 1 - import wag bez możliwości edycji
# 2 - losowe wagi
train_type = sys.argv[4].lower()
...
if architecture == 'resnet50':
    ...
    if train_type == '0':
        ...
        pretrained_model = resnet50.ResNet50(weights=None,
                                             include_top=False,
                                             input_shape=(ROWS, COLS, 3)
                                             )
        pretrained_model.trainable = True
    ...
    if train_type == '0':
        ...
        pretrained_model = vgg16.VGG16(weights=None,
                                       include_top=False,
                                       input_shape=(ROWS, COLS, 3)
                                       )
        pretrained_model.trainable = True
    ...
    if train_type == '0':
        ...
        pretrained_model = xception.Xception(weights=None,
                                             include_top=False,
                                             input_shape=(ROWS, COLS, 3)
                                             )
        pretrained_model.trainable = True

perform_test(preprocess_input=preprocess_input, pretrained_model=pretrained_model)
